...
// Sidebar widths (16rem expanded / 3rem icon rail / 18rem mobile sheet) are
// declared as static `[--sidebar-width:*]` utilities on the wrapper below
// (03 §8.3: no inline style); a consumer-passed `style` prop still overrides
// them because it flows through {...props} as a real inline style.
...
function SidebarGroupLabel({
    ...
    return (
        <Comp
            ...
            className={cn(
                'flex h-10 shrink-0 items-center rounded-md px-3 text-2xs font-medium tracking-caps uppercase text-sidebar-foreground/70 uppercase ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-3.5 [&>svg]:shrink-0',
                className
            )}
            {...props}
        />
    ...
    return (
        <div
            ...
            <Skeleton
                ...
                // 03 §8.3: the random runtime width travels through a CSS custom
                // property; the spread keeps a literal `style` attribute out of the JSX.
                {...{
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
    return (
        <ul
            ...
            className={cn(
                'mx-3 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2.5 py-0.5 group-data-[collapsible=icon]:hidden',
                className
            )}
            {...props}
        />
