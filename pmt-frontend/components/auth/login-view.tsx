...
export function LoginView() {
   ...
   *
   * `proxy.ts` puts the path the visitor was trying to reach into `next`, so a
   * link straight to a project survives the sign-in it triggered. It is
   * attacker controlled, so it goes through `safeRedirect` rather than being
   * used as given: anyone can send a colleague `/login?next=https://evil...`.
   */
  ...
  useEffect(() => {
    if (currentUser || authStatus === "authenticated") {
      ...
      void userStore.loadCurrentUser({ force: true }).then((verifiedUser) => {
        if (verifiedUser) router.replace(redirectTo);
      });
      return;
    }
  ...
  }, [authStatus, currentUser, redirectTo, router]);
