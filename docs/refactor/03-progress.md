...
| 6.5  | Response DTOs: `projects` completed, plus `project-members`, `project-documents` | done        | `project-members` (2 dto files to 1) and `project-documents` (4 to 1). Both carry display objects, capabilities and a mapper with specs. `formatFileSize` moved to the server, gaining the gigabyte tier the frontend copy lacked |
| 6.6  | Response DTOs: `time-tracking`, `work-reports`                                   | in progress | `time-tracking` done: 6 dto files to 1, 13 response DTOs, `formatDuration` and `toHours` moved to the server. `autoStopIfExpired` became a discriminated union in both services, which fixed a real type hole                     |
